...
bool reversingDirection(byte actualDirection, byte intendedDirection) {
  if ( actualDirection == LEFT && intendedDirection == RIGHT ) { 
    return true; 
  }
  if ( actualDirection == RIGHT && intendedDirection == LEFT ) { 
    return true; 
  }
  if ( snakeDirection == UP && intendedDirection == DOWN )     { 
    return true; 
  }
  if ( snakeDirection == DOWN && intendedDirection == UP )     { 
    return true; 
  }
  return false;
}
